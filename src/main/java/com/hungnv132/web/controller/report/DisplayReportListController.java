...
@Controller
public class DisplayReportListController {

	final Logger logger = LogManager.getLogger(DisplayReportListController.class);
	...
	@RequestMapping(value = "/admin/reports/user/{userId}", method = RequestMethod.GET)
...
/*	@RequestMapping(value = "/report/get-reports-of-user/{userId}", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/admin/staff/get-all-reports", method = RequestMethod.POST)
	@ResponseBody
	public Pagination<ReportListForManagerForm> getAllReportsForAdmin(DatatableReportFormForAdmin form) {
		...
		logger.info("++++++++ order[0][dir]" + form.getOrder().get(0).get("dir"));
		 ...
		 logger.info("++++++++ status " + form.getReportStatus());
		

		Pagination<ReportListForManagerForm> pageReport = reportService.getReportPageForADMIN(form);
		return pageReport;
	}
	
	@RequestMapping(value = "/manager/reports", method = RequestMethod.GET)
	public String reportList(Model model, HttpSession session) {
		return "manager-report-list";
	}

	@RequestMapping(value = "/manager/get-all-reports", method = RequestMethod.POST)
	@ResponseBody
	public Pagination<ReportListForManagerForm> getAllReportsForManager(DatatableReportForm form) {
		logger.info("++++++++ draw " + form.getDraw());
		logger.info("++++++++ start " + form.getStart());
		logger.info("++++++++ length " + form.getLength());
		logger.info("++++++++ search[regex] " + form.getSearch().get(SearchKeys.regex));
		...
		Pagination<ReportListForManagerForm> pageReport = reportService.getReportPageForMANAGER(form);
		return pageReport;
	}
